...
% rotation augments; timestep augments per rotation augment
rotations = 10;
timesteps_per_rotation = 10;

% sample params 
dT = 3;                % prediction timestep in hours
max_forecast = 24;     % maximum forecast length

data_vec = zeros(length(wrfout_files(:))*rotations*timesteps_per_rotation,27);    % N x (length(x) + length(y))
...
for Case = 1:length(wrfout_files(:))
    ...
    % get wrfout file for case
    case_wrfout_file = wrfout_files(Case).name;

    % simulation start date and time
    simulation_start_date = ncreadatt(wrfout_dir+ "/" +case_wrfout_file,"/","SIMULATION_START_DATE");
    ...
    % fire arrival times
    tign_g = double(ncread(wrfout_dir+ "/" +case_wrfout_file,'TIGN_G'));
    ...
    % terrain height data
    zsf = double(ncread(wrfout_dir+ "/" +case_wrfout_file,'ZSF'));
    zsf = zsf(1:1200,1:1200);
    zsf = rot90(zsf);                                                      % rotate to align E-W & N-S

    % fuel category data
    nfuel_cat = double(ncread(wrfout_dir+ "/" +case_wrfout_file,'NFUEL_CAT'));
    ...
    for rot = 1:rotations
        ...
        rotated_tign_g(rotated_tign_g==0) = max(rotated_tign_g,[],'all');
        ...
        for tstep = 1:timesteps_per_rotation
            ...
            met_em_folder = case_wrfout_file(1:end-3)+ "_met_em";
            met_em_files = dir(met_em_dir+ "/" +met_em_folder+ "/*.nc");
            met_em_idx_start = 1;
            ...
            UU1 = double(ncread(met_em_dir+ "/" +met_em_folder+ "/" +met_em_file_1,"UU"));
            VV1 = double(ncread(met_em_dir+ "/" +met_em_folder+ "/" +met_em_file_1,"VV"));
            RH1 = double(ncread(met_em_dir+ "/" +met_em_folder+ "/" +met_em_file_1,"RH"));
            TT1 = double(ncread(met_em_dir+ "/" +met_em_folder+ "/" +met_em_file_1,"TT"));
            UU2 = double(ncread(met_em_dir+ "/" +met_em_folder+ "/" +met_em_file_2,"UU"));
            VV2 = double(ncread(met_em_dir+ "/" +met_em_folder+ "/" +met_em_file_2,"VV"));
            RH2 = double(ncread(met_em_dir+ "/" +met_em_folder+ "/" +met_em_file_2,"RH"));
            TT2 = double(ncread(met_em_dir+ "/" +met_em_folder+ "/" +met_em_file_2,"TT"));
            ...
            % Project wind back to u and v coordinates
            ...
            crop_row_col_start = floor(length(rotated_tign_g)/2) - 149;
            ...
            data_vec(sample_counter,4) = fire_area_curr/19000;             % normalize by ...
            data_vec(sample_counter,5) = fire_area_prev/14000;
...
% save
training_data = data_vec(1:140*rotations*timesteps_per_rotation,:);
testing_data = data_vec(140*rotations*timesteps_per_rotation+1:end,:);
save("/Users/bshaddy/CD3_lab_stuff/wildfire_research/wrf-sfire_training_data_w_terrain/predict_fire_area_from_ign_time_conditions_(Isaac)/training_data_for_predicting_scalar_fire_area_recursively.mat",'training_data');
save("/Users/bshaddy/CD3_lab_stuff/wildfire_research/wrf-sfire_training_data_w_terrain/predict_fire_area_from_ign_time_conditions_(Isaac)/testing_data_for_predicting_scalar_fire_area_recursively.mat",'testing_data');
